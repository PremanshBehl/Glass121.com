...
import { Star, ArrowLeft, Heart, Share2, ShoppingCart, ShieldCheck, AlertTriangle } from "lucide-react";
...
export default function ProductDetail() {
  ...
  const product = {
    ...
    reviews: 247,
    description:
      "High-strength safety glass processed by controlled thermal treatments to increase its strength compared with normal glass. Perfect for shower enclosures, glass doors, and table tops.",
    priceRange: "₹120 - ₹160",
    stock: "in-stock",
    safetyLevel: "HIGH",
    attributes: [
  ...
  return (
    <div className="bg-white min-h-screen">
            ...
            <div className="lg:col-span-7">
              <div className="bg-white rounded-2xl aspect-[4/3] relative overflow-hidden mb-4 group">
                ...
                <div className="absolute top-4 right-4 flex gap-2">
                  <button
                    onClick={() => {
                      const safeId = (id as string) || "default-id";
                      const isSaved = wishlistItems.some((item) => item.id === safeId);
                      if (isSaved) {
                        removeFromWishlist(safeId);
                      } else {
                        addToWishlist({
                          id: safeId,
                          name: product.name,
                  ...
                  >
                    <Heart
                      size={20}
                      className={
                        wishlistItems.some((item) => item.id === ((id as string) || "default-id")) ? "fill-danger" : ""
                      }
                    />
                  </button>
              ...
              <div className="flex gap-4 overflow-x-auto hide-scrollbar">
                {[1, 2, 3, 4].map((i) => (
                  <div
                    key={i}
                    className={`w-20 h-20 rounded-lg shrink-0 cursor-pointer border-2 transition-all ${
                      i === 1 ? "border-accent-cyan bg-gray-100" : "border-transparent bg-gray-200 hover:border-gray-400"
                    }`}
                  ></div>
                ))}
              </div>
            ...
            <div className="lg:col-span-5 flex flex-col">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="flex items-center text-accent-gold text-sm font-bold bg-white/5 px-2 py-0.5 rounded">
                  <Star size={14} fill="currentColor" className="text-accent-gold mr-1" /> {product.rating}{" "}
                  <span className="text-gray-500 font-normal ml-1">({product.reviews})</span>
                </span>
              </div>

              <h1 className="text-3xl md:text-4xl font-poppins font-bold text-white mb-4 leading-tight">{product.name}</h1>

              <p className="text-gray-400 font-inter leading-relaxed mb-6">{product.description}</p>
              ...
              <div className="bg-[#11141e] rounded-xl p-6 border border-white/5 mb-8">
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-3xl font-mono font-bold text-accent-gold">
                      {product.priceRange}
                      <span className="text-base text-gray-500 font-inter font-normal">/sq.ft</span>
                    </p>
                  </div>
                ...
                <div className="space-y-4 mb-6">
                  ...
                  <div className="flex justify-between items-center border-b border-white/5 pb-2">
                    ...
                    <span className="text-white font-mono bg-accent-gold/20 text-accent-gold px-2 py-0.5 rounded text-xs">
                      {product.attributes.find((a) => a.label === "Process")?.value}
                    </span>
                  </div>
                ...
                <div className="flex flex-col gap-3">
                  <Link
                    href="/estimates"
                    className="w-full bg-accent-cyan hover:bg-[#00bfe6] text-primary-dark font-bold py-3 rounded-lg flex justify-center items-center gap-2 transition-all shadow-[0_0_20px_rgba(0,217,255,0.2)]"
                  >
        ...
        <div className="container mx-auto px-4">
          <div className="max-w-5xl mx-auto">
            <div className="flex border-b border-gray-200 mb-8 overflow-x-auto hide-scrollbar">
              {["Specs & Details", "Vendor Comparison", "Reviews (247)", "Installation Guide"].map((tab) => (
                <button
                  key={tab}
                  className={`px-6 py-4 font-poppins font-semibold text-sm whitespace-nowrap border-b-2 transition-colors ${
                    tab === "Specs & Details" ? "border-accent-cyan text-primary-dark" : "border-transparent text-gray-500 hover:text-primary-dark"
                  }`}
                >
                  {tab}
                </button>
              ...
              <div>
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden">
                  {product.attributes.map((attr, i) => (
                    <div
                      key={i}
                      className={`flex p-4 ${i !== product.attributes.length - 1 ? "border-b border-gray-200" : ""} ${i % 2 === 0 ? "bg-gray-50" : "bg-white"}`}
                    >
                ...
                <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-right">
                        <div className="font-mono font-bold text-accent-gold text-lg">
                          ₹130<span className="text-xs text-gray-500">/sq.ft</span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="border border-gray-200 rounded-xl p-5">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <div className="text-right">
                        <div className="font-mono font-bold text-accent-gold text-lg">
                          ₹125<span className="text-xs text-gray-500">/sq.ft</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4 text-sm text-gray-600 mb-4">
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-4 z-40 lg:hidden flex gap-4 shadow-[0_-10px_20px_rgba(0,0,0,0.05)]">
        ...
        <button className="flex-1 bg-accent-cyan text-primary-dark font-bold rounded-lg">Get Estimate</button>
      </div>
    </div>
  );
}
