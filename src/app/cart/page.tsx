    ...
    <div className="container mx-auto px-4 py-12 max-w-5xl">
      ...
      {items.length === 0 ? (
        <div className="bg-[#11141e] border border-white/10 rounded-2xl p-12 text-center">
          ...
          <Link href="/products" className="bg-accent-cyan text-primary-dark font-bold px-6 py-3 rounded-lg hover:bg-[#00bfe6] transition-colors inline-block">
            Browse Products
          </Link>
        </div>
      ) : (
        <div className="flex flex-col lg:flex-row gap-8">
          <div className="flex-1 space-y-4">
            {items.map((item) => (
              <div key={item.id} className="bg-[#11141e] border border-white/10 rounded-xl p-4 flex flex-col sm:flex-row items-center gap-6">
                ...
                <div className="text-right w-full sm:w-auto mt-4 sm:mt-0 flex justify-between sm:block">
                  ...
                  <p className="text-white font-bold text-lg mb-2">₹{(item.price * item.quantity).toLocaleString()}</p>
                  <button 
            ...
            <div className="bg-[#11141e] border border-white/10 rounded-2xl p-6 sticky top-24">
              ...
              <div className="border-t border-white/10 pt-4 mb-8">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-2xl font-mono font-bold text-accent-gold">₹{(getTotal() * 1.18).toLocaleString()}</span>
                </div>
              </div>

              <Link href="/checkout" className="w-full bg-accent-cyan hover:bg-[#00bfe6] text-primary-dark font-bold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
